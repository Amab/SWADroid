...
public class NewPracticeSession extends Module {
	...
	static final int EXIT_DIALOG_ID = 4;
	...
	private boolean formModified() {
		if (initialDayStart != mDayStart || initialMonthStart != mMonthStart || initialYearStart != mYearStart
				|| initialDayEnd != mDayEnd || initialMonthEnd != mMonthEnd || initialYearEnd != mYearEnd
				|| initialHourStart != mHourStart || initialMinuteStart != mMinuteStart
				|| initialHourEnd != mHourEnd || initialMinuteEnd != mMinuteEnd) {
			if (etSite.getText().toString() != "" || etDescription.getText().toString() != "")
				return true;
			else
				return false;
		} else
