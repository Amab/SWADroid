    ...
    public String getSQLAddField(Field field) {
        if (field != null) {
            ...
            if (field.getType().equals("multilanguage")) {
                ArrayList<String> langs = DataFramework.getInstance().getLanguages();
                for (int j = 0; j < langs.size(); j++) {
                    out += "ALTER TABLE " + mName + " ADD " + field.getName() + "_" + langs.get(j)
                            + field.getSQLAddField() + ";";
                }
            } else {
                out = "ALTER TABLE " + mName + " ADD " + field.getName() + " " + field
                        .getSQLAddField();
            }
            return out;
        }
    ...
    public Field getField(String name) {
        ...
        if (res == null) {
            throw new NullPointerException(
                    "No exist field '" + name + "' in table '" + mName + "'");
        }
        return res;
    }
