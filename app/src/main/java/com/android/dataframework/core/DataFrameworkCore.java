    ...
    public void open(Context context, String namePackage, ArrayList<Table> tables)
        ...
        try {
            ...
            if (tables.size() == 0) {
                ...
                mTables = tables;
                mCtx = context;
                int idTables = context.getResources()
                        .getIdentifier(namePackage + ":xml/tables", null, null);
                    ...
                    while (eventType != XmlPullParser.END_DOCUMENT) {
                        ...
                        if (eventType == XmlPullParser.START_TAG) {
                            if (x.getName().equals("database")) {
                                ...
                                mDataBaseVersion = Integer
                                        .parseInt(x.getAttributeValue(null, "version"));
                                if (x.getAttributeValue(null, "languages") != null) {
                                    boolean isFirst = true;
                                    StringTokenizer tokens = new StringTokenizer(
                                            x.getAttributeValue(null, "languages"), "|");
                                    while (tokens.hasMoreTokens()) {
                                        ...
                                        if (Locale.getDefault().getLanguage().equals(token)) {
                                            mCurrentLanguage = token;
                                        }
                                    }
                                    System.out.println(
                                            "Lenguaje por defecto usado en la base de datos: "
                                                    + mCurrentLanguage);
                                }
                                ...
                                if (x.getAttributeValue(null, "to-string") != null) {
                                    currentTable
                                            .setToString(x.getAttributeValue(null, "to-string"));
                                }
                                if (x.getAttributeValue(null, "backup") != null) {
                                    if (x.getAttributeValue(null, "backup").equals("no")) {
                                        currentTable.setBackup(false);
                                    }
                                }
                                if (x.getAttributeValue(null, "new-in-version") != null) {
                                    currentTable.setNewInVersion(Integer.parseInt(
                                            x.getAttributeValue(null, "new-in-version")));
                                }
                            } else if (x.getName().equals("field")) {
                                currentField = new Field(x.getAttributeValue(null, "name"));
                                if (x.getAttributeValue(null, "type") != null) {
                                    currentField.setType(x.getAttributeValue(null, "type"));
                                }
                                if (x.getAttributeValue(null, "foreign-table") != null) {
                                    currentField.setForeignTable(
                                            x.getAttributeValue(null, "foreign-table"));
                                }
                                ...
                                if (x.getAttributeValue(null, "size") != null) {
                                    currentField.setSize(
                                            Integer.parseInt(x.getAttributeValue(null, "size")));
                                }
                                if (x.getAttributeValue(null, "default") != null) {
                                    currentField
                                            .setTextDefault(x.getAttributeValue(null, "default"));
                                }
                                if (x.getAttributeValue(null, "new-in-version") != null) {
                                    currentField.setNewInVersion(Integer.parseInt(
                                            x.getAttributeValue(null, "new-in-version")));
                                }
    ...
    public void saveInitialValues() throws XmlPullParserException, IOException {
        try {
            for (int i = mDataBaseOldVersion + 1; i <= mDataBaseVersion; i++) {
                int idInitialValues = mCtx.getResources()
                        .getIdentifier(mPackage + ":xml/initialvalues_v" + i, null, null);
    ...
    public void backup(String file, boolean forceBackup)
        ...
        for (int i = 0; i < tableCount; i++) {
            ...
            if (t.isBackup() || forceBackup) {
                ...
                while (!c.isAfterLast()) {
                    osw.append("<row table=\"" + tableName + "\" id=\"" + c
                            .getString(c.getColumnIndex(DataFramework.KEY_ID)) + "\">\n");

                    for (int j = 0; j < fieldCount; j++) {
                        Field field = fields.get(j);
                        if (c.getString(c.getColumnIndex(field.getName())) != null) {
                            osw.append("<field name=\"" + field.getName() + "\" value=\"" + c
                                    .getString(c.getColumnIndex(field.getName()))
                                    .replace("\"", "&quot;") + "\" />\n");
                        }
                    }
                ...
                if (add(t.getNewInVersion(), oldVersion, newVersion)) {
                    ...
                    for (int j = 0; j < fieldCount; j++) {
                        ...
                        if (add(f.getNewInVersion(), oldVersion, newVersion)) {
                            ...
                            if (sql != null) {
                                System.out.println(
                                        "(onUpgrade) ADD FIELD: " + t.getName() + "(" + f.getName()
                                                + ")");
                                db.execSQL(sql);
                            }
                        }
